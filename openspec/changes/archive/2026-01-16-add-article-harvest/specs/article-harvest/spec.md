...
The system SHALL provide a new module named `article-harvest` under `modules/` that operates independently of the existing `article-ingest` module.
...
### Requirement: File-First Storage With Later SQLite Index
The system MUST support ingesting and querying articles using only filesystem data, and MAY introduce a SQLite index later without breaking the file-based workflow.

#### Scenario: Query without SQLite
- **WHEN** a user runs a query command before SQLite is introduced
...
#### Scenario: Failfast on source failure
- **WHEN** a source fetch fails
- **THEN** the failure is recorded and the system does not attempt alternative retrieval methods for that source
