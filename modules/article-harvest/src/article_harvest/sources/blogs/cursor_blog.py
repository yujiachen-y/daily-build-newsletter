...
def source() -> Source:
    return Source(
        ...
        kind="blog",
        method="html",
        fetch=fetch_cursor_blog,
    )


def fetch_cursor_blog(ctx: FetchContext) -> list[BlogItem]:
    html = get_text(ctx.session, CURSOR_BLOG_URL)
    entries = _extract_entries(html)
    if not entries:
        raise FetchError("Cursor Blog list empty")

    items: list[BlogItem] = []
    for entry in entries[:CURSOR_BLOG_LIMIT]:
        item = _fetch_article(ctx, entry)
        if item:
            items.append(item)

    if not items:
        raise FetchError("Cursor Blog returned no items")
    return items
...
def _extract_article_link(article: Tag) -> str | None:
    ...
    if not href.startswith("/blog/") or href.startswith("/blog/topic/"):
        return None
    return urljoin(CURSOR_BLOG_URL, href)
...
def _extract_published_at(soup: BeautifulSoup) -> str | None:
    ...
    if time_tag:
        ...
        if candidate:
            return _normalize_date(str(candidate))
    return None
